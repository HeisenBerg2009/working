...
const filterHouses = () => {
  ...
  const filteredHouses = houses.filter((house) => {
    const matchesSearch = house.title.toLowerCase().includes(searchValue);

    const matchesPrice =
      selectedPrice === "all" || house.price <= Number(selectedPrice);
...
search.addEventListener("keyup", filterHouses);
